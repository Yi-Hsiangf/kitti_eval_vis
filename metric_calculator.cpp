...
void Metric_calculator::get_matched_frame_with_distance(vector<string>label_list,vector<string> pred_list) {
    ...
    for(int file_idx=0;file_idx<label_list.size();file_idx++) {
        ...
        for(int gt_idx=0;gt_idx<gt_labels.size();gt_idx++) {
            ...
            store_matched_in_distance_map(file_idx, gt_labels[gt_idx][3], matched);
        }
    }
}
...
void Metric_calculator::get_matched_frame_with_distance(vector<string>label_list,vector<string> pred_old_list, vector<string> pred_new_list) {
    ...
    for(int file_idx=0;file_idx<label_list.size();file_idx++) {
        ...
        for(int gt_idx=0;gt_idx<gt_labels.size();gt_idx++) {
            ...
            store_matched_in_distance_map(file_idx, gt_labels[gt_idx][3], matched_old, matched_new);
        }
    }
}

void Metric_calculator::store_matched_in_distance_map(int file_idx, float distance, bool matched){
...
void Metric_calculator::store_matched_in_distance_map(int file_idx, float distance, bool matched_old, bool matched_new){
        int r_distance = int(distance);
        int m_distance;
        if(distance > 0) 
            m_distance = r_distance - r_distance % 10; // 69 -> 60
...
vector<int> Metric_calculator::get_unmatched_pred(bool is_new_model) {
    if(is_new_model)
...
void  Metric_calculator::show_matched_in_distance(bool is_compared_two_model) {
    ...
    for(int distance=-90;distance<=-10;distance+=10) {
        if(!is_compared_two_model)
            ...
            cout << "  distance from " << distance << " to " << distance - 10 << " in new model: " 
            << pred_new_distance_to_number_map[distance] << "/" << gt_distance_to_number_map[distance] << " in old model: " 
    ...
    if(!is_compared_two_model)
        ...
        cout << "  distance from -10 to 10 in new model: " << pred_new_distance_to_number_map[0] << "/" << gt_distance_to_number_map[0] << 
        " in old model: " << pred_old_distance_to_number_map[0] << "/" << gt_distance_to_number_map[0] << endl;
